...
async function main() {
  await Promise.all([
    ...
    esbuild.build({
      ...SHARED_OPTIONS,
      entryPoints: [path.join(ROOT, 'src/process/resources/builtinMcp/googleIntegrationServer.ts')],
      outfile: path.join(ROOT, 'out/main/builtin-mcp-google-integration.js'),
    }),
    esbuild.build({
      ...SHARED_OPTIONS,
