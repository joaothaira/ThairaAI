...
if (!gotTheLock) {
  console.warn('[ThairaAI] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[ThairaAI] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[ThairaAI] Creating main window...');
  ...
  console.log(`[ThairaAI] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[ThairaAI] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[ThairaAI] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[ThairaAI] Renderer did-finish-load');
      showWindow();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[ThairaAI] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[ThairaAI] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[ThairaAI] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[ThairaAI] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[ThairaAI] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[ThairaAI] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[ThairaAI] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[ThairaAI] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[ThairaAI] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[ThairaAI] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[ThairaAI] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[ThairaAI] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[ThairaAI] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[ThairaAI:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[ThairaAI] Failed to restore zoom factor:', error);
...
// Register thairaai:// as the default protocol client
...
// macOS: handle thairaai:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[ThairaAI] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[ThairaAI] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[ThairaAI] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[ThairaAI] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[ThairaAI] quit (exitCode=${exitCode})`);
});
