...
const BUILTIN_CHANNEL_TYPES = new Set(['telegram', 'lark', 'dingtalk', 'weixin', 'wecom', 'whatsapp', 'slack', 'discord']);
...
const ChannelModalContent: React.FC = () => {
  ...
  useEffect(() => {
    const unsubscribe = channel.pluginStatusChanged.on(({ status }) => {
      ...
      } else if (status.type === 'whatsapp') {
        setWhatsappPluginStatus(status);
    ...
    try {
      if (enabled) {
        if (!whatsappPluginStatus?.hasToken) {
          Message.warning(t('settings.whatsapp.configureFirst', 'Please save Server URL, Instance Name, and JWT Token first'));
        ...
        if (result.success) {
          ...
          Message.error(result.msg || t('settings.whatsapp.disableFailed', 'Failed to disable WhatsApp channel'));
        }
      }
    } catch (error: unknown) {
      Message.error(error instanceof Error ? error.message : String(error));
  ...
  const channels: ChannelConfig[] = useMemo(() => {
    const telegramChannel: ChannelConfig = {
      ...
      description: t('settings.channels.telegramDesc', 'Chat with ThairaAI assistant via Telegram'),
      status: 'active',
    ...
    const larkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.larkDesc', 'Chat with ThairaAI assistant via Lark or Feishu'),
      status: 'active',
    ...
    const dingtalkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.dingtalkDesc', 'Chat with ThairaAI assistant via DingTalk'),
      status: 'active',
    ...
    const weixinChannel: ChannelConfig = {
      ...
      description: t('settings.channels.weixinDesc', 'Chat with ThairaAI assistant via WeChat'),
      status: 'active',
    ...
    const wecomChannel: ChannelConfig = {
      ...
      description: t('settings.channels.wecomDesc', 'Chat with ThairaAI assistant via WeCom (Enterprise WeChat)'),
      status: 'active',
    ...
    const whatsappChannel: ChannelConfig = {
      ...
      description: t('settings.channels.whatsappDesc', 'Chat with ThairaAI assistant via WhatsApp'),
      status: 'active',
    ...
    const comingSoonChannels: ChannelConfig[] = [
      {
        ...
        description: t('settings.channels.slackDesc', 'Chat with ThairaAI assistant via Slack'),
      ...
      },
      {
        ...
        description: t('settings.channels.discordDesc', 'Chat with ThairaAI assistant via Discord'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
  ...
  const channelGuideText = t('settings.webui.featureChannelsDesc', {
    defaultValue: 'Connect Telegram, Lark, and DingTalk to interact with ThairaAI from IM apps.',
  });
