...
function buildCustomAgentInfo(
  ...
  const localeKey = locale.startsWith('pt') ? 'pt-BR' : 'en-US';
...
function inferLegacyAssistantInfo(
  ...
  const builtinByName = ASSISTANT_PRESETS.find((preset) =>
    matchesAssistantName(extractedName, [preset.id, preset.nameI18n['pt-BR'], preset.nameI18n['en-US']])
  );
  if (builtinByName) {
  ...
  const customByName = customAgents?.find((agent) =>
    matchesAssistantName(extractedName, [agent.id, agent.name, agent.nameI18n?.['pt-BR'], agent.nameI18n?.['en-US']])
  );
  if (customByName) {
