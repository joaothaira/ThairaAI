...
const AddPlatformModal = ModalHOC<{
  ...
  const isLocalEndpoint = useMemo(() => {
    try {
      const url = new URL(actualBaseUrl || '');
      ...
      <div className='pt-4px pb-12px'>
        <Form form={form} layout='vertical' className='[&_.arco-form-item]:mb-12px [&_.arco-form-item:last-child]:mb-0'>
          ...
          <Form.Item
            ...
            required={!isBedrock && !isLocalEndpoint}
            rules={[{ required: !isBedrock && !isLocalEndpoint }]}
            field={'apiKey'}
            extra={
              ...
              suffixIcon={
                <Search
                  onClick={async (e) => {
                    ...
                    // For Gemini and local endpoints, no apiKey check needed
                    if (!isGemini && !isLocalEndpoint && !apiKey) {
