...
import { isLocalProvider } from '@/common/utils/modelCapabilities';
...
const AionrsModelSelector: React.FC<{
  ...
  const currentModelToolWarning = useMemo(() => {
    if (!currentModel) return false;
    const matchedProvider = modelConfig?.find((p) => p.id === currentModel.id);
    if (!matchedProvider || !isLocalProvider(matchedProvider)) return false;
    return hasSpecificModelCapability(matchedProvider, currentModel.useModel, 'function_calling') !== true;
  }, [currentModel, modelConfig]);
              ...
              <Menu.ItemGroup title={provider.name} key={provider.id}>
                {models.map((modelName) => {
                  ...
                  const showToolWarning =
                    ...
                    hasSpecificModelCapability(provider, modelName, 'function_calling') !== true;
                  ...
                  return (
                    ...
                    >
                      <div className='flex items-center gap-8px w-full'>
                        ...
                        <span>{modelName}</span>
                        {showToolWarning && (
                          <Tooltip content={t('conversation.welcome.modelNoToolCallWarning')} position='right'>
                            <Attention theme='filled' size='12' className='text-warning shrink-0 ml-auto' />
                          </Tooltip>
                        )}
                      </div>
                    </Menu.Item>
    ...
    >
      ...
      >
        <span className='flex items-center gap-6px min-w-0'>
          ...
          {currentModelToolWarning && (
            <Tooltip content={t('conversation.welcome.modelNoToolCallWarning')} position='top'>
              <Attention theme='filled' size='12' className='text-warning shrink-0' />
            </Tooltip>
          )}
        </span>
      </Button>
