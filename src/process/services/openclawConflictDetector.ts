 ...
 * as ThairaAI Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with ThairaAI credentials
 */
...
/**
 * Check if OpenClaw Telegram channel conflicts with ThairaAI credentials
 */
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: OpenClaw ${platformName} channel is using the same credentials as ThairaAI.`,
    ``,
    `This means messages are being handled by OpenClaw, not ThairaAI Channels.`,
    `Switching agents in ThairaAI will have no effect.`,
    ``,
