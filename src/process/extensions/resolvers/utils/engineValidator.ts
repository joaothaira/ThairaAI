...
export function validateEngineCompatibility(extension: LoadedExtension): EngineValidationResult {
  ...
  // Check ThairaAI core version compatibility
  if (engine?.thairaai) {
    if (!satisfiesVersion(AIONUI_VERSION, engine.thairaai)) {
      result.valid = false;
      result.issues.push(
        `Extension "${extension.manifest.name}" requires ThairaAI ${engine.thairaai} but current version is ${AIONUI_VERSION}`
      );
    }
  }
