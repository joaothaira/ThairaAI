...
export class GeminiAgent {
  ...
  private initClientEnv() {
    ...
    if (this.authType === AuthType.USE_OPENAI) {
      ...
      // Local inference servers (Ollama, LM Studio) don't require an API key.
      // The OpenAI SDK requires a non-empty string, so use 'ollama' as a placeholder.
      const apiKey = (() => {
        const k = getCurrentApiKey();
        if (k) return k;
        try {
          const h = new URL(baseUrl || '').hostname;
          if (h === 'localhost' || h === '127.0.0.1' || h === '::1') return 'ollama';
        } catch {
          // ignore
        }
        return k;
      })();
      fallbackValue('OPENAI_API_KEY', apiKey);
      return;
    }
   ...
   * + client.ts "Please continue." mechanism). ThairaAI does NOT retry at this layer
    ...
    return processGeminiStreamEvents(
      ...
      .then(async () => {
        if (toolCallRequests.length > 0) {
          ...
          // Remap tool name aliases before dispatch (e.g. Ollama models call "google:search")
          this.remapToolNames(toolCallRequests);
  ...
  /**
   * Remap tool names from model-specific aliases to the canonical ThairaAI registry names.
   * Ollama models trained on Gemini data call "google:search" or "search" instead of
   * "google_web_search". Mutating in-place avoids reconstructing the objects.
   */
  private remapToolNames(requests: ToolCallRequestInfo[]): void {
    ...
    for (const req of requests) {
      const canonical = aliases[req.name];
      if (canonical) {
        console.log(`[GeminiAgent] Remapping tool "${req.name}" → "${canonical}"`);
        req.name = canonical;
      }
      // Normalize google_web_search args: Gemini-trained models pass `queries` (array)
      // but the ThairaAI tool schema requires a single `query` string.
      const toolName = canonical ?? req.name;
      if (toolName === 'google_web_search' && !req.args.query && req.args.queries) {
        const queries = req.args.queries;
        const first = Array.isArray(queries) ? queries[0] : String(queries);
        if (first) {
          console.log(`[GeminiAgent] Normalizing google_web_search args: queries[] → query="${first}"`);
          req.args = { ...req.args, query: first };
          delete req.args.queries;
        }
      }
    }
  }
