...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[ThairaAI] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[ThairaAI] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[ThairaAI] 数据迁移失败:', error);
  }
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[ThairaAI] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
  ...
  if (existsSync(builtinSkillsDir)) {
    try {
      ...
      console.warn(`[ThairaAI] Failed to sync builtin skills directory:`, error);
    }
  }
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[ThairaAI] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[ThairaAI] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[ThairaAI] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
          ...
          console.warn(`[ThairaAI] Failed to copy skill file ${skillFile}:`, error);
        }
      }
    } else {
...
const ensureBuiltinMcpServers = async (): Promise<void> => {
  try {
    ...
    if (changed) {
      ...
      console.log('[ThairaAI] Built-in MCP servers ensured');
    }
...
const cleanupOrphanedHealthCheckConversations = async () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[ThairaAI] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[ThairaAI] Failed to cleanup orphaned health-check conversations:', error);
  }
};
...
const initStorage = async () => {
  ...
  try {
    ...
    console.error('[ThairaAI] Failed to initialize default MCP servers:', error);
  }
  ...
  try {
    ...
    console.error('[ThairaAI] Failed to initialize builtin assistants:', error);
  }
...
export const loadSkillsContent = async (enabledSkills: string[]): Promise<string> => {
  ...
  for (const skillName of enabledSkills) {
    ...
    try {
      ...
      console.warn(`[ThairaAI] Failed to load skill ${skillName}:`, error);
    }
  }
