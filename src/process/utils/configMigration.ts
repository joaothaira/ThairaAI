...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[ThairaAI] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[ThairaAI] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[ThairaAI] Config migration from Electron failed:', error);
  }
}
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[ThairaAI] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[ThairaAI] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[ThairaAI] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[ThairaAI] IMPORT_CONFIG_FROM failed:', error);
  }
}
