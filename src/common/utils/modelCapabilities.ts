...
export const CAPABILITY_PATTERNS: Record<ModelType, RegExp> = {
  ...
  function_calling: /gpt-4|claude-3|gemini|qwen|deepseek|gemma[3-9]|llama3\.1|mistral-nemo/i,
...
/**
 * Returns true when the provider's baseUrl points to a local inference server
 * (Ollama, LM Studio, etc.) running on localhost.
 */
export const isLocalProvider = (provider: { baseUrl?: string }): boolean => {
  try {
