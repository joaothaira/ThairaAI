...
**支持的 Agent：** 内置 Agent（零配置） • Claude Code • Codex • Qwen Code • Goose AI • OpenClaw • Augment Code • CodeBuddy • Kimi CLI • OpenCode • Factory Droid • GitHub Copilot • Qoder CLI • Mistral Vibe • Nanobot • Thaira CLI（aionrs，AionUi 自带的 Rust 后端服务） • Snow CLI • Kiro • Hermes Agent • Cursor Agent 等
...
- **Leader 统筹编排** — Leader 分配、追踪并汇总结果；支持的后端包括 Claude Code、Codex、Hermes Agent、Gemini、Snow CLI 和 Thaira CLI
...
- **支持的后端** — Claude Code、Codex、Gemini、Snow CLI、Thaira CLI（aionrs）；其他具备 `mcpCapabilities.stdio` 的 ACP 后端自动支持
...
| AWS Bedrock 凭证                         | 通过 Thaira CLI（aionrs）的 Bedrock 驱动 Agent |
